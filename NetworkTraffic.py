...
def extract_website(url):
    # Initialize BeautifulSoup and requests libraries
    try:
        response = requests.get(url)
        soup = BeautifulSoup(response.content, "html.parser")

        # Extracting the website name from the parsed HTML
        website_name = soup.title.string or urlparse(url).netloc

        # Checking and returning the website name
        if website_name:
    ...
    if IP in packet:
        ...
        if protocol == "TCP":
            if packet.haslayer("Raw"):
                # if packet has raw layer, get raw data and decode it
...
# removing duplicate packets or unknown values
